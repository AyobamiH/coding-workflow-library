...
When the ledger status is `PR ready for merge approval`, `scripts/run-next --allow pr-merge` may merge a verified workflow-authored PR only after rechecking the auth user, repo access, exact changed files, PR state, `MERGEABLE` state, PR checks, reviewed head SHA, and repo-local workflow deployment evidence. It must not admin-merge, bypass protection, merge failing/pending checks, merge a changed head, or merge untrusted commits. After merge it must continue to `POST_MERGE_VERIFY` and only record `COMPLETED` after exact merge commit, local validation, remote alignment, ledger update, and run-record update pass.
...
- deploy/release/production/secret/destructive permission not already granted;
...
- Do not run deploys, migrations, database mutations, external API calls, releases, tags, merges, or closes without objective authority for that consequence class and the route-specific safety gates.
- Do not switch branch, stash, reset, clean, restore, commit, push, pull, or delete files without explicit permission.
