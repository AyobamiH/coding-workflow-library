...
| SOPS + age secret adapter | Adopt narrowly | John rejected the subscription-backed 1Password direction and selected a reputable open-source local path. `scripts/sops-age-secret-access` owns tool and identity readiness, non-decrypting encrypted-file validation, explicit output-suppressed `sops exec-env --pristine`, and precise capability blockers. `scripts/secret-bundles` adds only the proven multi-consumer manifest, coverage, delivery, and retirement layer. Terminal decryption, plaintext materialisation, provider mutation, and generic brokering remain rejected. |
...
SOPS and age are external open-source tools, not bundled package dependencies. Private age identities remain outside source. The observed local contract uses separate workstation and recovery recipients; independent recovery storage and a download/decryption round trip have been proven without publishing identity material. Synthetic tests prove permission and refusal paths, while real read-only GitHub checks and one non-force publication prove bounded credential injection with output suppressed and without cached GitHub authentication.

The purpose-bundle layer is not a second provider. It maps one legacy dotenv source into exact encrypted consumer boundaries, validates one-to-one name coverage, and permits only declared child basenames. Real manifests, ciphertext, evidence, local paths, and identities remain private runtime state.
